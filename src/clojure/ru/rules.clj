...
(defn modify-instances [inss]
  (retract-instances inss)
(assert-instances inss))
...
(defn display [mess]
  (let [dis (if-let [dss (seq (p/cls-instances "Display"))]
                ...
                (let [d (p/crin "Display")]
                  ...
                  d))
       src (p/sv dis "source")]
  (p/ssv dis "source" (str mess  "<br>" src))
  (.show p/*prj* dis)))
...
(defn print-facts [hm inst]
  (rete.core/facts))

(defn start-stop-clock [hm inst]
  (if (nil? clock/TIMER)
  (clock/start-clock)
  (clock/stop-clock)))
...
(defn filter-facts
  ([conds]
  (filter-facts (rete/fact-list) conds))
([cls conds]
  (if (empty? conds)
    (rete/fact-list cls)
    (let [filt (fn [acm [s p v :as spv]]
                    (rete/facts-with-slot-value cls s p v acm))
            [s p v] (first conds)
            acm1 (rete/facts-with-slot-value cls s p v)]
        (reduce filt acm1 (rest conds))))))

(defn do-for-fact [cls conds fun]
  (if-let[fact (first (filter-facts cls conds))]
  (fun fact)))

(defn do-for-all-facts
  ([conds fun]
  (doseq [f (filter-facts conds)]
    (fun f)))
([cls conds fun]
  (doseq [f (filter-facts cls conds)]
    (fun f))))

(defn collect-for-all-facts
  ([conds fun]
  (for [f (filter-facts conds)]
    (fun f)))
([cls conds fun]
  (for [f (filter-facts cls conds)]
    (fun f))))
